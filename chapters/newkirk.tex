...
\ChapterDOI{10.5281/zenodo.6393760}
...
On this analysis, \REF{ex:newkirk:14} has the preliminary structure in \figref{fig:fromex:newkirk:32}.
...
\begin{figure}%32
\caption{Syntax of example \REF{ex:newkirk:14}\label{fig:fromex:newkirk:32}}
...
\draw[->,semithick,dashed, overlay] (v1.south) to[out=250,in=180,looseness=2] node[near start,below]{\small m-merger} ($(reduceme3.west)+(0,0.5em)$);
\end{forest}}
\end{figure}
...
\begin{figure}%33
    \caption{Syntax of example \REF{ex:newkirk:11}\label{fig:fromex:newkirk:33}}
    \resizebox{\textwidth}{!}{\begin{forest}
    fairly nice empty nodes
    [TP
...
\section{Acquaintance relations}\label{sec:newkirk:4}\largerpage
...
\ea%37
    \label{ex:newkirk:37} \textit{Context:}
...
In base-generated prolepsis, the evaluated object binds the embedded \agre, and in turn the evaluated object is bound by the proleptic object in the matrix clause. Therefore the modified tree for \REF{ex:newkirk:11} is in \figref{fig:fromex:newkirk:39}.
...
\begin{figure}%39
\caption{Partial syntax of  example \REF{ex:newkirk:11}\label{fig:fromex:newkirk:39}}
\begin{forest}
fairly nice empty nodes
% [TP
	% [DP,name=specTP [Jack,name=Jack,roof] ]
...
\begin{figure}%40
\caption{Syntax of example \REF{ex:newkirk:14}\label{fig:fromex:newkirk:40}}
\resizebox{\textwidth}{!}{\begin{forest}
fairly nice empty nodes
% [TP
	% [DP,name=specTP [Jack,name=Jack,roof] ]
...
For Middle Dutch the matrix nominal is licensed by case marking, but on the analysis \citep{VanKoppenEtAl2016} it is in Spec,CP, although it has not been moved there. In German, prolepsis often feeds further movement that would otherwise be degraded \citep{Salzmann2017Chapter}.
...
\begin{tabbing}
 \textsc{caus}\hspace{1ex}\= Causative \kill
 {\textsc{appl}}  \>  applicative\\
  {c} followed by a number \hspace{1ex} noun class marker\\
 {\textsc{caus}} \> Causative \\
 {\textsc{fv}}  \>  Final vowel\\
 {\textsc{inv}} \> Inverse voice \\
