...
Lubukusu also marks reflexivity on the verb, where an invariant reflexive marker (REFL) occurs in the same position as the OM. A pronoun which takes noun class agreement matching its antecedent may also cooccur with the REFL, which surfaces as \ol{i-} regardless of the noun class of its antecedent. The REFL alone is sufficient to establish reflexivity, so the agreeing anaphor is optional. The Lubukusu REFL is also analyzed as an incorporated pronoun in line with Lubukusu's object markers, given its similar syntactic behavior.
...
The \agre\ pronoun can also occur without an accompanying REFL, in which case it cannot take a local antecedent, but is allowed to take a discourse antecedent:
...
Constructions with \ol{khu-mu-eene} in the matrix clause are insensitive to locality, whereas the construction with the REFL/OM cliticized to the matrix verb is sensitive to island boundaries, here shown with the REFL on matrix verb:
...
\begin{itemize}
	...
	\item The proleptic object as a reflexive marker (REFL) without an accompanying applicative or preposition
\end{itemize}
	...
	[{}
		...
		[$v$P
			[\sout{DP},name=specvP [\sout{Jack},name=vPJack,roof]]
			 [{}
				[$v$ [\ol{-i-}\\REFL,name=reduceme3] [\ol{a-i-many-il-e}\\know,name=v1]]
				[VP
	...
	% [{}
		...
		[$v$P
			[\sout{DP},name=specvP [John,name=vPJack,roof]]
			 [{}
				[$v$ [\ol{-i-}\\REFL,name=reduceme3] [\ol{many-il-e}\\know,name=v1]]
				[VP
					[V [\sout{\ol{many-il-e}}\\\sout{know},name=V1]]
					[SAP
							[Speaker]
...
{APPL}  &  applicative\\
 ...
 {FV}  &  Final vowel\\
 {OM}  &  Object Marker (typically followed by noun class number)\\
 \end{tabularx}
\begin{tabularx}{.45\textwidth}{lQ}
 {PREP}  &  Preposition\\
 {REFL}  &  Reflexive Marker\\
 {SM}  &  Subject Marker (typically followed by noun class number)\\
 {TNS}  &  Tense
\end{tabularx}
